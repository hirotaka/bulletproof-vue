...
<template>
  <div>
    <div v-if="discussions.error.value" class="mb-4 text-sm text-destructive" role="alert">
      {{ discussions.error.value.message }}
    </div>

    <div v-if="discussions.isPending.value" class="flex justify-center p-8">
      <USpinner />
    </div>

    <UTable
v-else :data="discussions.data.value.data" :columns="columns" :pagination="discussions.data.value.meta
        ? {
      ...
      <template #cell-view="{ entry }">
        <NuxtLink
:to="`/app/discussions/${entry.id}`" class="text-slate-600 hover:text-slate-900"
